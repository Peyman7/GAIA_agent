    ...
    def __call__(self, question: str, task_id: str = None) -> str:
        ...
        try:
            ...
            for msg in result_state.get("messages", []):
                print(f"{type(msg)} → {getattr(msg, 'role', None)}: {getattr(msg, 'content', None)}")
                print("=====================")
            print("🛠️  Tools used during run:")
            for msg in result_state.get("messages", []):
                if isinstance(msg, ToolMessage):
                    print(f"🔧 Tool Output: {msg.name} → {msg.content}")
            memory = result_state.get("memory", {})
            model_answer = memory.get("model_answer") or self._extract_final_answer(result_state)
            print(f"✅ Final Answer: {model_answer}")
            return model_answer or "(No answer returned)"
        except Exception as e:
            print(f"⚠️ Error running agent: {e}")
            traceback.print_exc()
            return "AGENT_ERROR"

    def _extract_final_answer(self, result_state: dict) -> str:
        for msg in reversed(result_state.get("messages", [])):
            if isinstance(msg, AIMessage):
                content = getattr(msg, "content", None)
                if content:
                    match = re.search(r"FINAL ANSWER\s*:\s*(.*)", content, flags=re.IGNORECASE)
                    if match:
